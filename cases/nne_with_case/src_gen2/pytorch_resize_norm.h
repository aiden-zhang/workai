...
__global__ void roi_yu122rgba_pad_kernel(uint8_t* __restrict__ in, uchar4* __restrict__ out, 
    ...
    int h_idx = (int)blockIdx.y - pad_h;
    bool h_run = h_idx >= 0 && h_idx < roi_h;
    uint8_t *u = in + in_w * in_h;
    uint8_t *v = u + in_w * in_h / 4;
    int y_offset = (h_idx + roi_h_start) * in_w + roi_w_start;
    int uv_offset = ((h_idx + roi_h_start) >> 1) * (in_w >> 1);
    ...
    if (w_idx < out_w) {
        ...
        if (h_run && w_run) {
            float3 a;
            a.x = in[y_offset + w_idx - pad_w];
            int uv_w = (w_idx - pad_w + roi_w_start)>>1;
            a.y = u[uv_offset + uv_w];
            a.z = v[uv_offset + uv_w];
            if (full_range) {
        ...
        int out_idx = blockIdx.y * out_w + w_idx;
        out[out_idx] = out_tmp;
    }
}
...
__global__ void rgba_horizontal_resize_kernel(uint8_t* __restrict__ in, uchar4* __restrict__ out,
    ...
    for (int w_idx = threadIdx.x; w_idx < out_w; w_idx += blockDim.x) {
        ...
        if (w_idx < out_w) {
            int xmin = horiz_bounds[0];
            int3 ss = {1 << (PRECISION_BITS - 1), 1 << (PRECISION_BITS - 1), 1 << (PRECISION_BITS - 1)};
            kernel_coeff.horiz_mad_coeffs_step(&ss, (uchar3*)(in + h_idx * in_w * 3), horiz_coeffs, horiz_bounds[0], in_w, Int2Type<0>());
