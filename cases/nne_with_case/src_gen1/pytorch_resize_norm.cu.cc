...
void launch_rgba_hori_resize(uint8_t* in, uchar4* out, int in_w, int in_h, int out_w, int out_h, 
        float filter_scale, int ksize, cudaStream_t stream, bool align, Int2Type<1>) {
}

template <int KSIZE>
void launch_rgba_hori_resize(uint8_t* in, uchar4* out, int in_w, int in_h, int out_w, int out_h, 
        float filter_scale, int ksize, cudaStream_t stream, bool align, Int2Type<KSIZE>) {
    if (ksize == KSIZE) {
        dim3 hori_block(256, 1, 1);
        if (out_w < 512) hori_block.x = out_w;
        dim3 hori_grid(1, out_h, 1);
        ...
        if (align) {
            rgba_horizontal_resize_kernel<KSIZE, true><<<hori_grid, hori_block, hori_sm_size, stream>>>(
                    in, out, in_w, in_h, out_w, out_h, filter_scale);
        } else {
            rgba_horizontal_resize_kernel<KSIZE, false><<<hori_grid, hori_block, hori_sm_size, stream>>>(
                    in, out, in_w, in_h, out_w, out_h, filter_scale);
        }
    } else {
        launch_rgba_hori_resize(in, out, in_w, in_h, out_w, out_h, filter_scale, ksize, stream, align, Int2Type<KSIZE - 2>());
    }
}
...
void RGBBilinearResizeCropNormPlaneV2(uint8_t *in_buf, float *out_buf, uchar4 *ws, int in_w, int in_h,
    ...
    launch_rgba_hori_resize(in_buf, rgba_buf, in_w, in_h, ws_w, ws_h, horiz_filterscale, horiz_ksize,
            stream, align, Int2Type<MAX_KSIZE>());
