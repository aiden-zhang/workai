...
/**
 *  @file  image_proc.h
 *  @brief Full APIs which support in this demo.
 */
...
/// @param align_w The stride of height of input image.
/// @param stream CU kernel run in the stream.
...
/// @brief Rotate RGB image with specific rotate mode.
/// @param in_buf The input buffer allocate in device memory.
///               The size is (in_w * in_h * 3) * sizeof(uint8_t).
/// @param out_buf The output buffer allocate in device memory.
///               The size is (out_w * out_w * 3) * sizeof(float).
/// @param in_w The width of input image.
/// @param in_h The height of input image.
/// @param rotate_mode The rotate angle of clockwise which include 90 degree, 180 degree, 270 degree.
/// @param stream CU kernel run in the stream.
void RGBRotate(uint8_t *in, uint8_t *out, int in_w, int in_h, int rotate_mode, cudaStream_t stream);



/// @brief Affine transformation info structure.
/// @param roi_w_start The start index of ROI area in the width dimension of intput image.
/// @param roi_h_start The start index of ROI area in the height dimension of intput image.
/// @param roi_w The width of ROI area in the width dimension of intput image.
/// @param roi_h The height of ROI area in the height dimension of intput image.
...
/// @brief Affine transformation batch of rgb image with bilinear interpolation.
///        This API is suitable for a lot of small rgb images compare with RGBAffine API.
/// @param input The input buffer allocate in device memory.
///               The size is (in_w * in_h * 3) * sizeof(uint8_t).
/// @param output The output buffer allocate in device memory.
///               The size is (out_w * out_h * 3 * batch) * sizeof(uint8_t).
/// @param batch Number of input images.
/// @param out_batch_offset The offset of per out image.
/// @param in_w The width of input image.
/// @param in_h The height of input image.
/// @param param The information of per affine transformation.
/// @param stream CU kernel run in the stream.
...
/// @brief multiple batch resize info structure.
/// @param in_w The width of input image.
/// @param in_h The height of input image.
/// @param img_w The width of resized image.
/// @param img_h The height of resized image.
...
/// @param pad_w The offset of width dimension of resized image in output image.
/// @param pad_h The offset of height dimension of resized image in output image.
/// @param w_scale The scale factor of width, float(in_w)/float(img_w).
/// @param h_scale The scale factor of height, float(in_h)/float(img_h).
typedef struct {
    int16_t in_h;
    int16_t in_w;
    int16_t out_h;
...
/// @brief Multiple batch resize RGB/BGR image with bilinear interpolation and normalization.
///        Output format is RRRGGGBBB / BBBGGGRRR.
///        Normalization function is:
///        output[channel] = (input[channel] * scale - mean[channel]) * standard[channel]
/// @param in_buf The input buffer allocate in device memory.
...
///               The size is (w_out * h_out * 3) * sizeof(float).
/// @param batch Number of input images.
/// @param input_batch_offset The stride of input per batch.
/// @param output_batch_offset The stride of output per batch.
/// @param param The information of per batch.
/// @param mean1, mean2, mean3, std1, std2, std3, scale
///        The parameter in "output[channel] = (input[channel] * scale - mean[channel]) * std[channel]".
/// @param pad1, pad2, pad3 The padding value of output.
/// @param channel_rev True mean rgb convert to bgr or bgr convert to rgb.
...
/// @brief multiple roi resize info structure.
/// @param roi_w_start The start index of ROI area in the width dimension of intput image.
/// @param roi_h_start The start index of ROI area in the height dimension of intput image.
/// @param roi_w The width of ROI area in the width dimension of intput image.
/// @param roi_h The height of ROI area in the height dimension of intput image.
...
typedef struct {
    ...
    float w_scale;
    float h_scale;
} multi_roi_resize_param;

/// @brief Multiple roi resize RGB/BGR image ROI area with bilinear interpolation and normalization.
///        Output format is RRRGGGBBB / BBBGGGRRR.
///        Normalization function is:
///        output[channel] = (input[channel] * scale - mean[channel]) * standard[channel]
/// @param in_buf The input buffer allocate in device memory.
