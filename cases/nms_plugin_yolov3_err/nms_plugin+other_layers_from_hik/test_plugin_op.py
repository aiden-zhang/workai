...
plugin_tvm_so=os.path.join(BasePath,"/home/gezishuai/hik_dl/build/libhikdl_nn.so")
TVM_TVM_REGISTER_SO_NAME=os.path.join(BasePath,"/home/gezishuai/hik_dl/build/libhik_tvm_op.so")
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".rlym") as f:
        ...
        with nne.Builder() as builder, nne.Parser() as parser:
            ...
            if serialize_file != None and deseralize:
                print('------------deserialize---------')
                file = open(serialize_file, 'rb')
                engine = nne.deserialize(file.read())
                file.close()
            else:
...
def test_hk_nms_all_plugin():
    ...
    b0 = np.load('./b0.npy').reshape(2,-1)
    b0 = np.round(np.divide(b0[0],b0[1]))

    b1 = np.load('./b1.npy').reshape(2,-1)
    b1 = np.round(np.divide(b1[0],b1[1]))

    b2 = np.load('./b2.npy').reshape(2,-1)
    b2 = np.round(np.divide(b2[0],b2[1]))

    b3 = np.load('./b3.npy').reshape(2,-1)
    b3 = np.round(np.divide(b3[0],b3[1]))

    data_main_input = relay.ones((1,64,20,32), dtype="float32")
    input = relay.var('input',shape=[1,64,20,32], dtype="float32")
    input_data = np.loadtxt('conv100.txt',dtype ="float32").reshape(1,64,20,32)
    # pdb.set_trace()
    data0 = dl.relay.op.qnn.quantize(input, output_scales=[0.125], output_zero_points=[0],out_dtype="int8")
    ...
    data2 = dl.relay.op.qnn.quantized_conv2d( 
        [data0, _expr.const(w0.astype('int8')), _expr.const(b0.astype('int32'))], 
        channels=32,
        kernel_size=(1, 1),
        data_layout="NCHW",
        out_dtype="float16",
    ...
    data4 = dl.relay.op.qnn.quantize(data3, output_scales=[0.0625], output_zero_points=[0],out_dtype="int8")
    ...
    data6 = dl.relay.op.qnn.quantized_conv2d(
        [data4, _expr.const(w1.astype('int8')), _expr.const(b1.astype('int32'))],
        channels=32,
        kernel_size=[1, 1],
        data_layout="NCHW",
        out_dtype="float16", 
    ...
    data9 = dl.relay.op.qnn.quantized_conv2d(
        [data7,_expr.const( w2.astype('int8')), _expr.const(b2.astype('int32'))],
        channels=32, 
        kernel_size=(3, 3), 
        data_layout="NCHW",
        out_dtype="float16",
    ...
    data153_var = dl.relay.op.qnn.quantized_conv2d(
        [data14, _expr.const( w3.astype('int8')), _expr.const(b3.astype('int32'))],
        channels=65, 
        kernel_size=(1, 1), 
        data_layout="NCHW", 
        out_dtype="float16", 
    ...
    # 1. need specify
    const227 = np.asarray([1.36,4.82,1.46,1.32,2.28,1.54,3.44,2.12,8.74,3.24]).reshape([5,2]).astype('float32')
    const227 = relay.const(const227)
    # const228 = 5
    # const228 = 40

    # 2. need to specify im_info_var
    # im_info_var = relay.ones((1,3),dtype="float32")
    im_info_var = relay.const(np.asarray([320,512,1]).astype('float32'),dtype='float32')
    # 3. need to specify data153_var
    # data153_var = relay.ones((1, 65, 20, 32),dtype="float32")
 
    # 4. need to specify how to take
    take1=np.asarray([0,1,2,3,
                    13,14,15,16,
                    26,27,28,29,
                    39,40,41,42,
                    52,53,54,55]).astype('int32')

    take2=np.asarray([4,17,30,43,56]).astype('int32')

    take3=np.asarray([5,6,7,8,9,10,11,12,
                    18,19,20,21,22,23,24,25,
                    31,32,33,34,35,36,37,38,
                    44,45,46,47,48,49,50,51,
                    57,58,59,60,61,62,63,64]).astype('int32')

    data154 = relay.take(data153_var, indices=relay.const(take1), axis=1, batch_dims=0, mode="clip")
    ...
    data161_boxes = tvm.get_global_func("hik_dl.op._make.custom_boxes_plugin") (data158, const227, data159, data160, "NHWC", 0)
    data162 = relay.take(data153_var, indices=relay.const(take2), axis=1, mode="fast")
    data163 = relay.take(data153_var, indices=relay.const(take3), axis=1, mode="fast")
    ...
    data200 = _op.reshape(data167, [1, 1, 5, 20, 32]) * data166
    ...
    sorted_scores_idx = tvm.get_global_func("hik_dl.op._make.custom_filter_sort_single_input")(
        # data169,
        # _expr.Tuple([data200]),
        _expr.Tuple([data167, data168]),
    ...
    data177_boxes_gather = tvm.get_global_func( "hik_dl.op._make.custome_non_max_suppression_gather_boxes")(data175_boxes, data172_sorted_idx, data174_sort_size)
    ...
    data179_boxes_ids_count = tvm.get_global_func("hik_dl.op._make.custom_non_max_suppression")(data177_boxes_gather,
        data178_sorted_scores,
        max_output_size_per_class_var,
        iou_threshold_var,
        score_threshold_var,
    ...
    data182_csum_value = tvm.get_global_func("dl.hik_dl.op._make.custom_csum")(data180_count,-1,1)
    # data183 = tvm.get_global_func("dl.relay.op._make.custom_combine_non_max_suppression_post")(
    data183 = tvm.get_global_func("dl.hik_dl.op._make.custom_combine_non_max_suppression_post")(
    ...
    data184, data187, data185,data_tmp = _expr.TupleWrapper(data183, 4)
    ...
    data_tmp = relay.cast(data_tmp, dtype="float32")  #/ *ty = Tensor[(1, 8), float32] * /;
    ...
    output = _expr.Tuple((data_tmp, data188, data189, data190))
    ...
    # output = relay.cast(data180_count, "float32")
    net = relay.Function([input], output)
    ...
    output = evaluate_with_nne(mod, params,"0", serialize_file='./nms_engin.slz', deseralize=True)
