...
#define LAUNCH_RGB_RESIZE_NORM_PLANE_KERNEL(is_bilinear, align_in_w, channel_rev) \
    ...
    switch (h_out % 4) { \
        case 0: \
            ...
            rgb_resize_ROI_norm_kernel<is_bilinear, align_in_w, 1 << ALIGN4B, channel_rev><<<grid, block, roi_w * 3 * 2 + 8 * 2, stream>>>( \
        ...
        case 2: \
            ...
            rgb_resize_ROI_norm_kernel<is_bilinear, align_in_w, 1 << ALIGN2B, channel_rev><<<grid, block, roi_w * 3 * 2 + 8 * 2, stream>>>( \
                       in_buf, out_buf, h_in, w_in, h_out, w_out, img_h, img_w, pad_h, pad_w, h_scale, w_scale, \
                       roi_h_start, roi_w_start, roi_h, roi_w, scale, mean1, mean2, mean3, std1, std2, std3, pad1, pad2, pad3); \
            break; \
        default: \
            grid.x = is_bilinear ? h_out / (1 << ALIGN1B) : grid.x; \
            rgb_resize_ROI_norm_kernel<is_bilinear, align_in_w, 1 << ALIGN1B, channel_rev><<<grid, block, roi_w * 3 * 2 + 8 * 2, stream>>>( \
                       in_buf, out_buf, h_in, w_in, h_out, w_out, img_h, img_w, pad_h, pad_w, h_scale, w_scale, \
                       roi_h_start, roi_w_start, roi_h, roi_w, scale, mean1, mean2, mean3, std1, std2, std3, pad1, pad2, pad3); \
            break; \
    }
