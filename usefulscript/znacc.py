...
def eval_acc(
    ...
    ex = create_executor(mod, origin_model, "vm", target)
...
def drive_acc(args):
    ...
    if args.input_data_file is not None:
        input_data_dict = {key: np.load(args.input_data_file)  for key in list(input_shape_dict)}
    else:
        input_data_dict = {key: np.ones(input_shape_dict[key], dtype=data_type) for (key, data_type) in zip(list(input_shape_dict) , _)}
    # pdb.set_trace()
    target_output = eval_acc(
        mod,
        args.model,
        input_data_dict,
        target=args.target,
    )
    if args.reference_target is not None:
        ...
        print(golden_output)
        compare_consine(golden_output, target_output)
    ...
    if(args.dumpOutput):
        if args.reference_target:
            np.save('targetoutput.npy', target_output)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', "--target", type=str, default="onnx", help="target backend to infer,support: nne, llvm, onnx")
    parser.add_argument("--reference-target", type=str, default=None, help="the target backend to yeild the golden") #tghe .onnx  model can specify onnx
    parser.add_argument("--input_data_file", type=str, default=None, help="input data file of npy")
