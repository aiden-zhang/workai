...
class Extractor:
    ...
    def _collect_new_io_core(self, original_io, io_names_to_extract, is_output=True):  # type: ignore
        original_io_map = self._build_name2obj_dict(original_io)

        if is_output and ([original_io_map.keys()] == [dict.fromkeys(io_names_to_extract).keys()]):
            return None
    ...
    def _collect_new_inputs(self, names):  # type: (List[Text]) -> List[ValueInfoProto]
        return self._collect_new_io_core(self.graph.input, names, False)  # type: ignore
    ...
    def _dfs_search_reachable_nodes(
            ...
            reachable_nodes,  # type: set[str]
    ):  # type: (...) -> None
        if node_output_name in graph_input_names:
            return
        for node in self.graph.node:
            if (*node.input,node.name,*node.output) in reachable_nodes:
                continue
            if node_output_name not in node.output:
                continue
            reachable_nodes.add((*node.input,node.name,*node.output))
    ...
    def _collect_reachable_nodes(
        ...
        nodes = [n for n in self.graph.node if (*n.input,n.name,*n.output) in reachable_nodes]
        return nodes
